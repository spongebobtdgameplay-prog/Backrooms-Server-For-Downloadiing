...
bool GameTextRenderer::Initialize()
{
#ifdef _WIN32
    if (Ready)
        return true;
...
void GameTextRenderer::Shutdown()
{
    for (auto& Pair : Cache)
    {
...
void GameTextRenderer::Resize(uint32_t NewWidth, uint32_t NewHeight)
{
    Width = std::max(NewWidth, 1u);
    Height = std::max(NewHeight, 1u);
}

bool GameTextRenderer::IsReady() const
{
    return Ready;
}

bool GameTextRenderer::CreateShader()
{
...
std::string GameTextRenderer::CacheKey(
...
GameTextRenderer::CachedText* GameTextRenderer::GetOrCreate(
    const std::string& Text,
    int PixelHeight,
    int Weight,
    float TrackingEm
)
{
    ...
    const std::string Key =
        CacheKey(Text, PixelHeight, Weight, TrackingEm);
    ...
    const int CharacterExtra =
        static_cast<int>(
            std::round(
                static_cast<float>(PixelHeight) * TrackingEm
            )
        );

    const wchar_t* Face =
        ResolveGameplayFontFace(DeviceContext);
    ...
    const int BitmapWidth = std::max(Size.cx + 16, 1L);
    const int BitmapHeight =
        std::max(
            static_cast<int>(Metrics.tmHeight) +
                static_cast<int>(Metrics.tmExternalLeading) + 16,
            1
        );
...
{
    CachedText* Entry =
        GetOrCreate(Text, PixelHeight, Weight, TrackingEm);

    return Entry != nullptr
        ? std::max(Entry->Width - 16, 0)
        : 0;
}

void GameTextRenderer::Draw(
    const std::string& Text,
    int X,
    int Y,
    int PixelHeight,
